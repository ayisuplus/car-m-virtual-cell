...
6: """[谨慎报数]以下数字全部出自 CT-0508 一期报告（Reiss 等，Nature Medicine 2025，doi:10.1038/s41591-025-03495-z）：共治疗 14 例；无 ≥3 级 CRS 或 ICANS；HER2 3+ 患者 44%（4/9）最佳疗效为疾病稳定；41% 可测量病灶出现肿瘤体积缩小——但按 RECIST v1.1 无客观缓解，所以我们不引用任何"肿瘤缩小百分比"作为疗效数字。
...
我们用神经 surrogate 替代这一步。实测（11 次计时中位数）:surrogate 推理约 {_SURR_US:.2f}µs/细胞，代表性 RK4 ODE 求解约 {_ODE_US:.2f}µs，约 {_SPEEDUP:.1f}× 提速;N≈55 细胞时每帧极化 <0.1ms，60fps 余量充足。
