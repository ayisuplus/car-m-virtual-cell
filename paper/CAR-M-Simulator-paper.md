...
**Training and validation metric.** The network is trained by a reproducible script (`scripts/train-surrogate.mjs`, fixed seed 20250706) on 30,000 synthetic samples generated by a biologically-motivated ODE polarization model, with inputs drawn uniformly over [0,1]⁶, for 100 epochs (Adam, batch 64), on an 80/20 train/validation split. On the held-out 20% it reaches **99.98%** per-sample discrete-label agreement on the thresholded phenotype in {M1, M2, MIXED}, with per-output MAE ≈ 0.0005 / 0.0001 / 0.001 and R² ≈ 1.0 / 1.0 / 0.997 for the M1, M2, and phagocytosis outputs respectively. We emphasise that these are single static, single-step metrics; the *trajectory-level* ODE-vs-surrogate error accumulated under the exponential-relaxation scheme of Section 4.2 is not yet characterised and is listed as planned validation (Section 7), because a high single-step accuracy does not by itself bound drift over a 40-minute run. In effect, the surrogate learns a fast, differentiable approximation of the ODE's steady-state map. We emphasise that the *ground truth* here is the ODE itself, not experimental data; calibrating both against measured single-cell profiles is future work (Section 6).
...
- **Checkpoint blockade is encoded in the phagocytosis module (encoded-and-visualised).** The CD47/SIRPα and CD24/Siglec-10 terms implement the "don't-eat-me" gate by construction, so the workbench lets a user *inspect* how toggling blockade changes the multiplier the model applies. With the corrected surrogate and dt-scaled event rates, the single default seed now shows the unblocked CD47 run accumulating fewer phagocytosis events than the blocked baseline (24 vs. 31 at t = 40 min; Fig. 3b), consistent with the mechanism's direction. This remains one seed; confirming robustness requires a multi-seed Monte-Carlo envelope (planned; Section 7), so we still report this as an encoded, inspectable behaviour rather than a demonstrated robust trend.
